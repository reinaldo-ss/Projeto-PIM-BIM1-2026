...
/* ===== Menu hamburguer (mobile ≤ 900px) =====
   Alterna o wrapper do menu e a animação do ícone. */
function toggleMenuMobile(botao) {
    const wrapper = botao.parentElement.querySelector('.menu-mobile-wrapper');
    if (!wrapper) return;
...
function fecharMenuMobile() {
    ...
    document.querySelectorAll('.menu-mobile-wrapper.aberto').forEach(w => w.classList.remove('aberto'));
}
